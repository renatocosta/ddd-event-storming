...
class ProjectReviewUseCaseTest extends AggregateRootTestCase
{
    ...
    public function setUp(): void
    {
        ...
        $reviewProjectEventHandler = new ReviewProjectEventHandler($orderRepository);
        $this->domainEventBus = $this->faker->eventBus($reviewProjectEventHandler);
    }
    ...
    public function test_should_fail_to_invalid_project()
    {
        ...
        $reviewProjectUseCase = new ReviewProjectUseCase($order, $orderRepository);
        $orderId = Guid::from(Uuid::uuid4());
        $this->given(new ReviewProjectInput($orderId), $reviewProjectUseCase)
            ->when(...$this->domainEventBus->getRaisedEvents())
            ->thenNothingShouldHaveHappened()
            ->expectToFail(new UnableToHandleProjectReports());
    }

    public function test_executing_a_project_review_usecase_successfully()
    {
        $order = new OrderEntity($this->domainEventBus);
